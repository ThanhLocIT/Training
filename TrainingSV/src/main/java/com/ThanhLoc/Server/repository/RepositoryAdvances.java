...
public interface RepositoryAdvances extends JpaRepository<Advances,Long> {
    @Query("SELECT " + "new com.ThanhLoc.Server.payload.Response.MoneyAdvancesResponse(b.employeeId,SUM(b.money))" + " FROM " + " Employees as a, Advances  b WHERE a.id = :id and a.id = b.employeeId and b.status =1 and EXTRACT(YEAR FROM b.date) = :year and EXTRACT(MONTH FROM b.date) = :month GROUP BY a.id")
